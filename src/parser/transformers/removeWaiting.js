...
export default function removeWaiting(map, list) {
  return list.map((gpx) => {
    return {
      ...gpx,
      points: removePoints(map, gpx.points),
    };
  });
}

//what are good seconds/meter values? Or should it only check after longs gaps in the gps time? (eg the tracking was paused)
function removePoints(map, points, seconds = 10, meters = 5) {
  ...
  for (let index = 1; index < points.length; index++) {
    ...
    const distanceDiff = distance(map, previous, current);
